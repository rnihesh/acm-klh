...
function VendorScorecardContent() {
  ...
  if (!gstin) {
    return (
      <PageShell title="Vendor Scorecard" description="Select a vendor">
        <div className="c-bg-card rounded-xl border c-border p-12 text-center">
          <ShieldAlert className="w-12 h-12 c-text-3 mx-auto mb-4" />
          <p className="c-text-2 font-medium mb-1">No vendor selected</p>
          <p className="c-text-3 text-sm">
            Go to{" "}
            <Link href="/risk" className="underline c-text-accent">
              Vendor Risk
            </Link>{" "}
            and select a vendor.
          </p>
...
export default function VendorScorecardPage() {
  return (
    <Suspense
      fallback={
        <PageShell title="Vendor Scorecard" description="Loading...">
          <div className="flex items-center justify-center py-20">
            <div className="animate-spin rounded-full h-8 w-8 border-2 border-t-transparent" style={{ borderColor: "var(--text-secondary)", borderTopColor: "transparent" }} />
          </div>
        </PageShell>
